...
WORKDIR /src
COPY ["WebApi_PostGreSQLDemo.csproj", "."]
RUN dotnet restore "./WebApi_PostGreSQLDemo.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./WebApi_PostGreSQLDemo.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# This stage is used to publish the service project to be copied to the final stage
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./WebApi_PostGreSQLDemo.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=true
...
ENTRYPOINT ["WebApi_PostGreSQLDemo.exe"]